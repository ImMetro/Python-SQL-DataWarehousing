...
for i in tables_maze:
    ...
    create_table_query = f'({", ".join([f"{col[3]} VARCHAR(255)" for col in schema]) + ", ValidFrom DATETIME2 GENERATED ALWAYS AS ROW START NOT NULL, ValidTo DATETIME2 GENERATED ALWAYS AS ROW END NOT NULL, PERIOD FOR SYSTEM_TIME (ValidFrom, ValidTo)"}) WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = {"dbo."+i+"_Maze_History"}))'
    create_table_query_list = list(create_table_query.split(", "))
    create_table_query_list[0] = create_table_query_list[0] + " NOT NULL PRIMARY KEY CLUSTERED"
    create_table_query = ",".join(create_table_query_list)
    
    smmcd3_cursor.execute(f'CREATE TABLE {i + "_Maze"} {create_table_query}')
    smmcd3_cursor.commit()
    print(f"Created {i} Table in Historical-SMMCD3...")
    
    destination_column_names = ', '.join([column.COLUMN_NAME for column in schema])
    ...
    try:
        ...
        insert_query = f'INSERT INTO {i+"_Maze"} ({destination_column_names}, ValidFrom, ValidTo) VALUES ({", ".join(["?" for _ in range(len(schema))])}, DEFAULT, DEFAULT)'
        smmcd3_cursor.executemany(insert_query, data)
        smmcd3_cursor.commit()
...
for i in tables_ced3:
    ...
    smmcd3_server_history   = "redacted"
    smmcd3_username_history = "redacted"
    smmcd3_password_history = "redacted"

    smmcd3_db_history   = 'DRIVER={SQL Server};SERVER='+smmcd3_server_history+';UID='+smmcd3_username_history+';PWD='+ smmcd3_password_history+';DATABASE=redacted'
    smmcd3_history      =  pyodbc.connect(smmcd3_db_history)
    print("SMMD3 History Connection: ", smmcd3_history)

    # ---- Cursor Setup ----
    ced3_cursor = ced3.cursor()
    smmcd3_cursor_history = smmcd3_history.cursor()
    ...
    #Create a Primary Key
    create_table_query = f'({", ".join([f"{col[3]} VARCHAR(255)" for col in schema]) + ", ValidFrom DATETIME2 GENERATED ALWAYS AS ROW START NOT NULL, ValidTo DATETIME2 GENERATED ALWAYS AS ROW END NOT NULL, PERIOD FOR SYSTEM_TIME (ValidFrom, ValidTo)"}) WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = {"dbo."+i+"_ced3_History"}))'
    create_table_query_list = list(create_table_query.split(", "))
    create_table_query_list[0] = create_table_query_list[0] + " NOT NULL PRIMARY KEY CLUSTERED"
    create_table_query = ",".join(create_table_query_list)
    
    smmcd3_cursor_history.execute(f'CREATE TABLE {i + "_ced3"} {create_table_query}')
    smmcd3_cursor_history.commit()
    print(f"Created {i} Table in Historical-SMMCD3...")
    ...
    insert_query = f'INSERT INTO {i+"_ced3"} ({destination_column_names}, ValidFrom, ValidTo) VALUES ({", ".join(["?" for _ in range(len(schema))])}, DEFAULT, DEFAULT)'
    smmcd3_cursor_history.executemany(insert_query, data)
    smmcd3_cursor_history.commit()
    ...
    ced3_cursor.close()
    ced3.close()
    smmcd3_cursor_history.close()
    smmcd3_history.close()
    print("Refreshing access tokens for the next table...")    
